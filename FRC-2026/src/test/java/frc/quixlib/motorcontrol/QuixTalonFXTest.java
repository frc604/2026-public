...
public class QuixTalonFXTest implements AutoCloseable {
  ...
  public void setup() {
    ...
    controller_follower =
        new QuixTalonFX(new CANDeviceID(1), controller, QuixTalonFX.makeDefaultConfig());
  ...
  @Test
  public void testSetSupplyCurrentLimit() {
    // Verify default supply current limit from config.
    assertEquals(40.0, controller.getConfiguration().CurrentLimits.SupplyCurrentLimit, kTol);

    // Set a new supply current limit.
    controller.setSupplyCurrentLimit(20.0);

    // Config should be updated immediately (local state).
    assertEquals(20.0, controller.getConfiguration().CurrentLimits.SupplyCurrentLimit, kTol);

    // Wait for the background executor to complete the CAN apply.
    Timer.delay(0.5);

    // Should still reflect the updated value.
    assertEquals(20.0, controller.getConfiguration().CurrentLimits.SupplyCurrentLimit, kTol);

    // Set another value to verify repeated calls work.
    controller.setSupplyCurrentLimit(30.0);
    assertEquals(30.0, controller.getConfiguration().CurrentLimits.SupplyCurrentLimit, kTol);
    Timer.delay(0.5);
    assertEquals(30.0, controller.getConfiguration().CurrentLimits.SupplyCurrentLimit, kTol);
  }
}
