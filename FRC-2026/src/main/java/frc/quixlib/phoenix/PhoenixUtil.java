...
public class PhoenixUtil {
  ...
  // For synchronized refresh.
  private static BaseStatusSignal[] m_signals = new BaseStatusSignal[0];

  public static void registerSignals(final BaseStatusSignal[] signals) {
    BaseStatusSignal.refreshAll(signals);
    BaseStatusSignal[] newSignals = new BaseStatusSignal[m_signals.length + signals.length];
    System.arraycopy(m_signals, 0, newSignals, 0, m_signals.length);
    System.arraycopy(signals, 0, newSignals, m_signals.length, signals.length);
    m_signals = newSignals;
  }

  public static StatusCode refreshAll() {
    if (m_signals.length > 0) {
      return BaseStatusSignal.refreshAll(m_signals);
    }
  ...
  public static boolean retryUntilSuccess(
    ...
    return retryUntilSuccess(setterFunction, () -> true, 8, description);
  }
  ...
  public static boolean retryUntilSuccess(
    ...
    return retryUntilSuccess(setterFunction, validatorFunction, 8, description);
  }
  ...
  public static boolean CANcoderConfigsEqual(
    ...
    if (!MathUtils.epsilonEquals(
        ...
        kEps)) {
      DriverStation.reportWarning(
          "[AbsoluteSensorDiscontinuityPoint] Expected: "
              + expected.MagnetSensor.AbsoluteSensorDiscontinuityPoint
              + ", Actual: "
              + actual.MagnetSensor.AbsoluteSensorDiscontinuityPoint,
          false);
