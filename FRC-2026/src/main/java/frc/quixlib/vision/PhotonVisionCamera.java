...
public class PhotonVisionCamera implements QuixIO, QuixVisionCamera {
  ...
  public void updateInputs() {
    ...
    if (latestResults.size() > 0) {
      m_inputs.latestResult = latestResults.get(latestResults.size() - 1);
    }
  ...
  public PipelineVisionPacket getLatestMeasurement() {
    ...
    if (!hasTargets) {
      return new PipelineVisionPacket(false, null, null, Optional.empty(), -1);
    }
    ...
    return new PipelineVisionPacket(
        ...
        result.getTimestampSeconds() - 0.01);
  }
}
