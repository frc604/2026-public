...
public class AutoFastRaceNoDepot implements AutoCommand {
  ...
  public AutoFastRaceNoDepot(
      final SwerveSubsystem swerve, final IntakeSubsystem intake, final boolean isLeft) {
    m_swerve = swerve;
    m_intake = intake;
    m_first = new QuikPlanSwervePartialTrajectoryReader("fast_race.csv", isLeft);
    m_second = new QuikPlanSwervePartialTrajectoryReader("fast_race_return.csv", isLeft);
  }
