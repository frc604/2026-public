...
def plan(quiet):
    ...
    sweep_start_pose = (8.2, 2.25, 0.25 * np.pi)
    qp.add_waypoint(
        sweep_start_pose,
        10,
    ...
    sweep_turnaround_pose = (8.2, 2.75, np.pi)
    qp.add_waypoint(
        sweep_turnaround_pose,
        10,
        # intermediate_constraints=[SpeedConstraint(2.5)],
        end_constraints=[PoseConstraint(sweep_turnaround_pose)],
    )

    sweep_end_pose = (7.2, 2.75, 1.25 * np.pi)
    qp.add_waypoint(
        sweep_end_pose,
        10,
