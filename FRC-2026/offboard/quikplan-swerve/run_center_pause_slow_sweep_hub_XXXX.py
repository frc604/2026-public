...
def plan(quiet):
    ...
    qp = QuikPlan(
        field,
        robot,
        constants.CENTER_PAUSE_SLOW_XXXX_POSE,
        [StoppedPoseConstraint(constants.CENTER_PAUSE_SLOW_XXXX_POSE)],
    ...
    sweep_turn_pose_A = (9, 4.3, 0)
    qp.add_waypoint(
        sweep_turn_pose_A, 10, end_constraints=[PoseConstraint(sweep_turn_pose_A)]
    )

    sweep_turn_pose_F = (9.7, 4.3, 0)
    qp.add_waypoint(
        sweep_turn_pose_F,
        20,
    ...
    sweep_turn_pose_G = (9.7, 2.5, -np.pi)
    qp.add_waypoint(
