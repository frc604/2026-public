...
def plan(quiet):
    ...
    after_bump_pose = (5.5, 2.5, 0.5 * np.pi)
    qp.add_waypoint(
        after_bump_pose,
        10,
    ...
    ball_clearance_pose = (7.0, 1.0, 0.5 * np.pi)
    qp.add_waypoint(
        ball_clearance_pose,
        10,
        end_constraints=[PoseConstraint(ball_clearance_pose)],
        end_action=Action(ActionType.DEPLOY_INTAKE),
    )

    sweep_start_pose = (8.4, 1.0, 0.5 * np.pi)
    qp.add_waypoint(
        sweep_start_pose,
        10,
    ...
    sweep_turnaround_pose = (8.4, 4, np.pi)
    qp.add_waypoint(
